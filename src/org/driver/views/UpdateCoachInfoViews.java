...
import org.driver.service.UpdateCoachInfoService;
...
public class UpdateCoachInfoViews extends JFrame {
	private static final Logger logger = Logger.getLogger(UpdateCoachInfoViews.class);
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UpdateCoachInfoViews frame = new UpdateCoachInfoViews();
		...
		button_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				try {
					...
					c_id=operation.readTxtFile(new File("temp\\CoachInfo.txt")).replace("null", "").trim();
				} catch (Exception e1) {
					logger.error("异常信息---获取文件内容t_id失败",e1);
				}
				GetCoachService getCoachService=new GetCoachService();
				Coach c=getCoachService.getById(c_id);
				...
				c.setC_tel(tel);
				c.setC_icon(icon);
				UpdateCoachInfoService updateCoachInfoService=new UpdateCoachInfoService();
				updateCoachInfoService.update(c);
